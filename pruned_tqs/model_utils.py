...
@torch.no_grad()
def sample(model, batch=10000, max_unique=1000, symmetry=None, param=None):
    """
    Parameters
    ----------
    ...
    produced_states, log_probs = model.generate_n_unique_states(
        param_value=param,
        batch_size=batch,
        system_size=model.system_size,
        num_unique=max_unique,
    ...
    if symmetry is not None:
        samples = symmetry.apply_random(produced_states)

    return samples, torch.exp(log_probs)  # (n, batch), (batch, )
...
def compute_psi(model, samples, symmetry=None, check_duplicate=True):
    ...
    # note log amp is 1/2 log prob
    log_amp = (
        model.evaluate_log_prob(
    ...
    # 1 should be fine here right? -- shouldnt matter
    log_phase = torch.ones_like(log_amp)
