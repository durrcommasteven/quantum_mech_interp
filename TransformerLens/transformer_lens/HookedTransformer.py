...
class HookedTransformer(HookedRootModule):
    ...
    def set_param(self, param_range, param=None):
        if param is None:
    ...
    def evaluate_log_prob(
        ...
        batch_size, system_size = state.shape

        if isinstance(param_values, (float, int)):
            param_values = torch.full(
                size=(batch_size,), fill_value=param_values, dtype=torch.float32
            )

        assert len(param_values.shape) == 1
        assert param_values.shape[0] == batch_size
        ...
        logits = self.forward(tokens).squeeze(-1)[:, :-1]

        # select those tokens which occurred
        # negate logit if token is 0
        state_logits = logits * (2 * state - 1)

        state_log_probs = torch.nn.functional.logsigmoid(state_logits)

        # now return the sum of these
        return einops.reduce(state_log_probs, "i ... -> i", "sum")

    @torch.inference_mode()
    def generate_state(
        self,
        param_values: torch.Tensor | tuple[int, float],
        system_size: int,
        init_spin_config: torch.Tensor | None = None,
        verbose: bool = False,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        # assumes stoquastic hamiltonian
        # that is, no phase output
        # return the samples states, and the log prob
        # for now, we aren't using kv cache, but we should
        ...
        num_new_tokens = system_size - (tokens.shape[-1] - 1)

        self.eval()
        for index in tqdm.tqdm(range(num_new_tokens), disable=not verbose):
            ...
            # note the output size is 1
            final_logits = logits[:, -1, 0]

            zero_one_samples = (
                torch.nn.functional.sigmoid(final_logits) > torch.rand_like(final_logits)
            ).to(torch.float32)

            zero_one_samples.to(devices.get_device_for_block_index(0, self.cfg))

            tokens = torch.cat([tokens, zero_one_samples.unsqueeze(-1)], dim=-1)

        # cut off the param value of the tokens
        tokens = tokens[:, 1:]

        # now use the last value of logits to get the log prob
        sampled_logits = logits[..., 0] * (2 * tokens - 1)

        sampled_logprobs = torch.nn.functional.logsigmoid(sampled_logits).sum(-1)
    ...
    def generate_n_unique_states(
        ...
        # assumes stoquastic hamiltonian, and a fixed param val
        # that is, no phase output
        # return the sampled states, the log probs
        # ensure at least num_unique unique samples
        # output tensor of shape (batch_size, system_size),
        # and log_probs of shape batch_size

        assert batch_size >= num_unique
        assert num_unique <= 2**system_size
        ...
        # we want to keep track of which params we're generating
        # first idx = current dummy index, second is the index of
        # the original param_values tensor

        while num_states < num_unique:
            cur_attempt += 1

            states, log_probs = self.generate_state(
                param_values=(batch_size - num_states, param_value),
                system_size=system_size,
                init_spin_config=init_spin_config,
                verbose=False,
            )
            ...
            unique_reprs = self.map_to_binary(unique_states)
            seen_mask = torch.isin(unique_reprs, output_state_reprs)

            unique_states = unique_states[~seen_mask]
            unique_log_probs = unique_log_probs[~seen_mask]
            unique_reprs = unique_reprs[~seen_mask]

            num_new_unique = unique_states.shape[0]

            output_states[num_states : num_states + num_new_unique, :] = unique_states
            output_log_probs[num_states : num_states + num_new_unique] = unique_log_probs
            output_state_reprs[num_states : num_states + num_new_unique] = unique_reprs
        ...
        if num_states == batch_size:
            return output_states, output_log_probs

        num_remainder = batch_size - num_states

        states, log_probs = self.generate_state(
            param_values=(num_remainder, param_value),
            system_size=system_size,
            init_spin_config=init_spin_config,
            verbose=False,
        )

        output_states[-num_remainder:, :] = states
        output_log_probs[-num_remainder:] = log_probs
