...
class MLP(CanBeUsedAsMLP):
    ...
    def freeze_weights(self):
        # Turn off the gradient updates to the weights.
        # This is used when we want to turn off and eventually
        # remove these from the model.
        self.W_in.requires_grad = False
        self.b_in.requires_grad = False
        self.W_out.requires_grad = False
        self.b_out.requires_grad = False

    def unfreeze_weights(self):
        # Turn on the gradient updates to the weights.
        self.W_in.requires_grad = True
        self.b_in.requires_grad = True
        self.W_out.requires_grad = True
        self.b_out.requires_grad = True
    ...
    def forward(
        ...
        assert 0 <= self.alpha <= 1, f"alpha value should be in [0,1], alpha={self.alpha}"

        return output * self.alpha + unaltered_x * (1 - self.alpha)
